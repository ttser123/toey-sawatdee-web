...
const ToolSkeleton = () => (
    <div className="w-full h-[600px] bg-slate-100 animate-pulse rounded-sm flex items-center justify-center">
        <span className="text-slate-400 font-mono text-xs uppercase tracking-widest">Booting YAML Architect...</span>
    </div>
);

const DockerComposeClientOnly = dynamic(
  () => import('@/components/tools/DockerComposeTool'),
  {
    ssr: false,
    ...
    <div className="grid grid-cols-1 gap-4 lg:gap-8">
      <section className="space-y-4">
        <h3 className="text-[10px] sm:text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] sm:tracking-[0.3em] font-mono">
          [ Mission Control: Docker Architect ]
        </h3>
        <DockerComposeClientOnly />
      </section>
    </div>
  );
}
