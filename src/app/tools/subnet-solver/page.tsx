...
const SubnetSolverClientOnly = dynamic(
  () => import('@/components/tools/SubnetSolver'),
  {
    ssr: false,
...
export default function SubnetSolverPage() {
  return (
    <div className="grid grid-cols-1 gap-4 lg:gap-8">
      <section className="space-y-4">
        <h3 className="text-[10px] sm:text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] sm:tracking-[0.3em] font-mono">
          [ Mission Control: Network Collision Solver ]
        </h3>
        <SubnetSolverClientOnly />
      </section>
    </div>
  );
}
